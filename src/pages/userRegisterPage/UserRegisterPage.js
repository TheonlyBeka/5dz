...
function UserRegisterPage(props) {
    ...
    const registerUser = (event) => {
        ...
        if(user.name.trim() === ""){
            dispatch(alertOn({message:"Enter name", variant:"danger"}))

            setTimeout(()=>{
                dispatch(alertOff())
            }, 3000)
        } else if (user.username.trim() === ""){
            dispatch(alertOn({message:"Enter username", variant:"danger"}))

            setTimeout(()=>{
                dispatch(alertOff())
            }, 3000)
        } else if (user.email.trim() === ""){
            dispatch(alertOn({message:"Enter Email", variant:"danger"}))
